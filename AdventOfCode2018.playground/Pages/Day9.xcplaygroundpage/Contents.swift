...
func part1() {
    ...
    for i in 1..<marbleCount {
        if i % 23 == 0 {
            let player = (i - 1) % playerCount
            points[player] += i

            let removalIndex = (currentIndex - 7 + circle.count) % circle.count
            points[player] += circle[removalIndex]

            circle.remove(at: removalIndex)
            currentIndex = removalIndex
        }
        else {
            let insertionIndex = (currentIndex + 1) % circle.count + 1
            circle.insert(i, at: insertionIndex)
...
func part2() {
    let playerCount = 452
    let marbleCount = 71250 * 100

    var points = Array(repeating: 0, count: playerCount)
    var currentNode = Node(value: 0)
    ...
    for i in 1..<marbleCount {
        if i % 23 == 0  {
            ...
            points[player] += nodeToRemove.value
